...
def test_start_turn_normalizes_color_and_sets_timestamp():
    ...
    with patch(
        "shatranj.domain.rules.BlitzClock.time.time", return_value=10.0
    ):
...
def test_end_turn_subtracts_elapsed_time_and_adds_increment():
    ...
    with patch(
        "shatranj.domain.rules.BlitzClock.time.time", return_value=15.5
    ):
        clock.end_turn()
...
def test_get_remaining_time_uses_live_clock_for_active_player():
    ...
    with patch(
        "shatranj.domain.rules.BlitzClock.time.time", return_value=24.0
    ):
